  ...
  <section class="main">
    ...
    <div class="container">
      <div class="main__section">
        <h2 class="main__title" v-if="getContacts.title">
          {{ getContacts.title }}
        </h2>
        <div
        ...
        <ul class="contact__wrapper" v-if="pageContent.linksData.length">
          <li
            class="contact__item"
            v-for="(link, index) in pageContent.linksData"
            :key="index"
          >
            <a
              v-if="link.mail"
              :href="'mailto: ' + link.content.link"
              class="contact__links link"
            >
              <img
                class="contact__social-img"
                :alt="link.content.text"
                :src="link.img.src"
              />
              {{ link.content.text }}
            </a>
        ...
        <Breadcrumbs
          v-if="getContacts.links.length"
          :links="getContacts.links"
        />
      </div>
    </div>
  </section>
